...
-define(TPKT_VRSN, 3).
-define(LINE_LENGTH, 1023). % (default value of afunix option 'recbuf') - 1
...
echo_test_1(SockOpts, EchoFun, Config0) ->
    ...
    ?line Config = [{echo_name, EchoPort}|Config0],
...
echo_packet(SockOpts, EchoFun, Opts) ->
    ...
    EchoName = ?config(echo_name, Opts),
    {ok, Echo} = afunix:connect(EchoName, SockOpts),
...
active_echo(Sock, Type, Packet, PacketEchos) ->
    ?line ok = afunix:send(Sock, Packet),
...
echo_server(ReplyTo) ->
    SocketName = socket_name("echo_server"),
    {ok, S} = afunix:listen(SocketName, [{active, false}, binary]),
    ReplyTo ! {echo_name, SocketName},
    echo_server_loop(S).

echo_server_loop(Sock) ->
    {ok, E} = afunix:accept(Sock),
    Self = self(),
...
echoer_loop(Sock) ->
    case afunix:recv(Sock, 0) of
	{ok, Data} ->
	    ok = afunix:send(Sock, Data),
	    echoer_loop(Sock);
	{error, closed} ->
	    ok
    end.
...
slow_echo_server() ->
    ...
    ?line receive
	      {echo_name, SocketName} ->
		  {ok, SocketName}
	  end.

slow_echo_server(ReplyTo) ->
    SocketName = socket_name("slow_echo_server"),
    {ok, S} = afunix:listen(SocketName, [{active, false}, {nodelay, true}]),
    ReplyTo ! {echo_name, SocketName},
...
socket_name(Name) ->
    SocketName = filename:join("/tmp",Name),
